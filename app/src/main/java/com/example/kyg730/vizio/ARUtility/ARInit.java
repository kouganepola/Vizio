...
public class ARInit {
    ...
    private void loadFromImage(ImageTracker tracker, String path)
    {
        ...
        tracker.loadTarget(target, new FunctorOfVoidFromPointerOfTargetAndBool() {
            ...
            public void invoke(Target target, boolean status) {
                Log.i("BBB", String.format("load target (%b): %s (%d)", status, target.name(), target.runtimeID()));
            }
        });
    }
    ...
    public boolean initialize()
    {
        ...
        DaoSession daoSession = DaoMaster.newDevSession(EasyAR.getApplicationContext(),"Vizio-db");
        book.__setDaoSession(daoSession);

        List<ARContent> arContentsList = book.getARContentList();
        for (ARContent arContent: arContentsList ){

            loadFromImage(tracker,EasyAR.getApplicationContext().getFilesDir()+arContent.getTargetPath());
